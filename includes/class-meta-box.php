...
abstract class Meta_Box {
	...
	public function __constructor() {
		$this->nothing_selected = __('-- Choose one --', MR_POTATOHEAD_TEXTDOMAIN );
												...
												)
	{
		...
		$ul .= '<button type="button" title="' . $this->get_tooltip( 'add_button' ) . '" class="' . $add_button_class . '" id="' . $name . ' add-button" formaction="javascript:void(0)">' . __( '+', MR_POTATOHEAD_TEXTDOMAIN ) . '</button>';
	...
	protected function null_option() {
		$option_tag = '<option value="">' .  __('--Choose One--', MR_POTATOHEAD_TEXTDOMAIN ) . '</option>';
		return $option_tag;
	}
