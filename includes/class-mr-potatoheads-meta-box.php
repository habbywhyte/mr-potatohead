...
class Mr_Potatoheads_Meta_Box extends Meta_Box {
	...
	public function meta_box_render() {
		...
		echo '<div class="mph_settings_container">';

		$this->section_heading(__('Mr_Potatoheads Settings', MR_POTATOHEAD_TEXTDOMAIN), 'mph-mb-content-settings');

        $this->text_input( __('Feet', MR_POTATOHEAD_TEXTDOMAIN),
            get_post_meta( $post_ID, 'mph_feet', true),
            'mph_feet',
            'feet.png'
        );
	...
	public function post_meta_save( $post_id ) {
		...
		$this->update_meta_text( $post_id, 'mph_feet');
	...
	protected function init_tooltips() {
		$tooltips = array(
			'add_button' => __( 'Click this button to add a new item to this list.', MR_POTATOHEAD_TEXTDOMAIN ),
			'meta_proposal_list' => __('For each price panel, enter date ranges during which a price will be in effect. Enter a price only for Sunday to set the price the same for all days of the week. Click the + button below to add a new price panel.', MR_POTATOHEAD_TEXTDOMAIN),
			'delete_proposal_button' => __('Click here to delete this proposal panel.', MR_POTATOHEAD_TEXTDOMAIN)
		);
	...
	public function remove_meta_boxes () {
		remove_meta_box('revisionsdiv', 'mrpotatoheads', 'norm');
		remove_meta_box('slugdiv', 'mrpotatoheads', 'norm');
		remove_meta_box('authordiv', 'mrpotatoheads', 'norm');
		remove_meta_box('postcustom', 'mrpotatoheads', 'norm');
		remove_meta_box('postexcerpt', 'mrpotatoheads', 'norm');
		remove_meta_box('trackbacksdiv', 'mrpotatoheads', 'norm');
		remove_meta_box('commentsdiv', 'mrpotatoheads', 'norm');
		remove_meta_box('pageparentdiv', 'mrpotatoheads', 'norm');
	}
